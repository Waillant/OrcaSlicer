                                        ...
                                        const CvtVI2VI     &cvt_from)
{
	...
	// Get the optional property map
	auto opt_map = patch_new.mesh.property_map<VI, VI>(patch_source_name);

	// Check if it has a value before using it
	if (opt_map.has_value()) {
	    CvtVI2VI &cvt = opt_map.value(); // Get the reference to the map
	    for (VI &vi : cvt) {
...
void priv::store(const CutMesh &mesh, const FaceTypeMap &face_type_map, const std::string& dir, bool is_filled)
{
    ...
    auto face_colors = mesh_.add_property_map<priv::FI, CGAL::Color>("f:color").value();
    for (FI fi : mesh.faces()) { 
...
void priv::store(const CutMesh &mesh, const ReductionMap &reduction_map, const std::string& dir)
{
    ...
    CutMesh &mesh_ = const_cast<CutMesh &>(mesh);
    auto vertex_colors = mesh_.add_property_map<priv::VI, CGAL::Color>("v:color").value();
